...
const Employee = (() => {
  ...
  function home() {
    ...
    const exams = DB.getExamsForEmployee(uid);
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div class="card" style="padding:20px">
          ...
          ${materials.slice(0, 3).map(m => {
            ...
            const thumb = m.cover
              ? `<div style="width:40px;height:40px;border-radius:8px;background-image:url('${m.cover}');background-size:cover;background-position:center;flex-shrink:0"></div>`
              : `<div style="width:40px;height:40px;border-radius:8px;background:var(--surface-2);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">${typeIconMap[m.type]}</div>`;
  ...
  function learnCenter() {
    ...
    const currentUserName = App.currentUser.name;

    return `
      <div class="page-header">
        <div>
          <h2>学习中心</h2>
          <div class="subtitle">浏览、搜索和下载培训资料 · 上传者可管理自己的资料</div>
        </div>
      ...
      <div class="toolbar">
        <div class="search-box">
          ...
          <input type="text" placeholder="搜索资料标题或描述..." value="${learnFilter.keyword}" oninput="Employee.setLearnFilter('keyword', this.value)" />
        </div>
        <span class="badge badge-gray">共 ${list.length} 项</span>
      </div>
      ${list.length === 0 ? emptyState('📚', '暂无可学习的资料，点击右上角上传') : `
      <div class="cards-grid">
        ${list.map(m => {
          const cat = DB.getCategoryById(m.categoryId);
          const p = progress.find(x => x.materialId === m.id);
          const isCompleted = p && p.completed;
          const canManage = m.uploader === currentUserName;
          return `
          <div class="material-card">
            <div class="material-thumb ${m.type}">
              ${m.cover
                ? `<img class="cover-img" src="${m.cover}" alt="${escapeAttr(m.title)}" /><span class="thumb-type-badge">${typeIconMap[m.type]} ${m.type.toUpperCase()}</span>`
                : `<div class="thumb-default">${typeIconMap[m.type]}</div>`}
            ...
            <div class="material-body">
              ...
              <div class="material-meta">
                ...
                <span>👤 ${escapeHtml(m.uploader || '未知')}</span>
              ...
              <div class="material-actions">
                ...
                ${canManage ? `<button class="btn btn-sm btn-secondary" onclick="Admin.materialModal('${m.id}')">✏️ 编辑</button>` : ''}
                ${canManage ? `<button class="btn btn-sm btn-danger" onclick="Employee.delMaterial('${m.id}')">🗑️</button>` : ''}
              </div>
            </div>
          </div>`;
        }).join('')}
  ...
  function openMaterial(id) {
    ...
    openModal({
      ...
      body: `
        ...
        ${FileUtil.canDownload(m) ? `<div style="margin-top:14px;text-align:center"><button class="btn btn-secondary" onclick="Employee.downloadMaterial('${m.id}')">⬇️ 下载主文件</button></div>` : ''}
        ${attachCount > 1 ? `
        <div style="margin-top:16px">
          <h4 style="margin-bottom:10px">📎 全部附件</h4>
          <div style="display:flex;flex-direction:column;gap:6px">
            ${m.attachments.map((a, i) => `
              <div style="display:flex;align-items:center;gap:8px;padding:10px;border:1px solid var(--border);border-radius:6px">
                <span style="font-size:18px">${typeIconMap[a.type] || '📎'}</span>
                <div style="flex:1">
                  <div style="font-weight:500">${escapeHtml(a.fileName || '附件' + (i+1))}</div>
                  <div style="font-size:12px;color:var(--text-muted)">${a.fileSize ? formatSize(a.fileSize) : '-'}</div>
                </div>
                ${(a.fileData || a.url || a.content) ? `<button class="btn btn-sm btn-secondary" onclick="Employee.downloadAttachment('${m.id}', ${i})">⬇️</button>` : ''}
              </div>
            `).join('')}
          </div>
        </div>` : ''}
  ...
  function delMaterial(id) {
    const m = DB.getMaterialById(id);
    if (!m) return;
    if (m.uploader !== App.currentUser.name) {
      toast('您只能删除自己上传的资料', 'error'); return;
    }
  ...
  function exams() {
    const uid = App.currentUser.id;
    // 只显示该员工被分配参加的已发布考试
    const list = DB.getExamsForEmployee(uid);
    const myRecords = DB.getRecordsByUser(uid);
    return `
      <div class="page-header">
        <div>
          <h2>考试中心</h2>
          <div class="subtitle">参加分配给你的考核，检验学习成果</div>
        </div>
      </div>
      ${list.length === 0 ? emptyState('📋', '暂无分配给你的考试') : `
      <div class="cards-grid">
        ${list.map(e => {
          ...
          // 检查时间限制
          const now = Date.now();
          const timeOk = (!e.startTime || now >= e.startTime) && (!e.deadline || now <= e.deadline);
          return `
          <div class="exam-card">
            ...
            <div class="exam-body">
              ...
              <div style="font-size:12px;color:var(--text-muted);margin-bottom:8px">
                ...
                ${e.randomOrder ? '· 随机出题' : ''}
                ${e.startTime ? `· 开始: ${formatDateTime(e.startTime)}` : ''}
                ${e.deadline ? `· 截止: ${formatDateTime(e.deadline)}` : ''}
              </div>
              <div class="exam-actions">
                ${inProgress ? `<button class="btn btn-sm btn-success" onclick="Employee.resumeExam('${e.id}')">▶️ 继续考试</button>` : ''}
                ${canTake && timeOk ? `<button class="btn btn-sm ${inProgress ? 'btn-secondary' : 'btn-primary'}" onclick="Employee.startExam('${e.id}')">${inProgress ? '重新开始' : '▶️ 开始考试'}</button>` : ''}
                ${!timeOk && canTake ? '<span class="badge badge-warning">未开放/已截止</span>' : ''}
                ${!canTake ? '<span class="badge badge-gray">次数已用完</span>' : ''}
  ...
  return {
    home, learnCenter, setLearnFilter, openMaterial, downloadMaterial, downloadAttachment, delMaterial,

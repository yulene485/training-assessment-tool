...
const Reports = (() => {
  ...
  function viewRecord(recordId) {
    ...
    const hasSubjective = r.hasSubjective;
    const subjectivePending = hasSubjective && !r.subjectiveGraded;
    const resultLabel = subjectivePending ? '⏳ 待评分' : (r.passed ? '🎉 通过' : '😕 未通过');

    openModal({
      title: '答题详情',
      size: 'xl',
      body: `
        <div class="result-hero ${subjectivePending ? '' : (r.passed ? 'pass' : 'fail')}" style="margin-bottom:16px;padding:20px">
          ...
          <div class="result-status" style="font-size:18px">${resultLabel}</div>
          <div class="result-meta" style="font-size:13px">
            ${escapeHtml(r.userName)} · ${escapeHtml(r.examTitle)} ${subjectivePending ? '· 客观题得分率 ' + pct + '%（主观题待评分）' : '· 得分率 ' + pct + '%'} ·
            答对 ${r.correctCount || 0}/${r.questionCount || details.length} 题 ·
            用时 ${Math.round(r.duration / 60)} 分钟 · 第 ${r.attempt} 次
          </div>
          ${subjectivePending ? '<div style="margin-top:10px;padding:12px;background:var(--warning-light);border-radius:8px;font-size:13px">⚠️ 含主观题待评分，请逐题评分后点击"确认评分"完成。</div>' : ''}
        </div>
        ${details.length === 0 ? '<div class="empty-state">该记录无详细答题数据</div>' : details.map((d, i) => {
          const isSubj = d.subjective || subjectiveTypes.includes(d.type);
          if (isSubj) {
            return `
            <div class="question-card" style="margin-bottom:12px;border-left:4px solid var(--warning)">
              <div class="question-num">
                <span class="q-num">第 ${i + 1} 题</span>
                <span class="badge badge-info q-type-badge">${typeLabels[d.type] || '主观题'}</span>
                <span class="q-score">${d.earned}/${d.score} 分</span>
                ${d.earned > 0 ? '<span class="badge badge-success">✓ 已评分</span>' : '<span class="badge badge-warning">⏳ 待评分</span>'}
              </div>
              ...
              <div style="margin-top:12px;padding:14px;background:var(--surface-2);border-radius:8px">
                <div style="font-weight:600;margin-bottom:8px">学员作答：</div>
                <div style="white-space:pre-wrap;font-size:14px;line-height:1.7">${escapeHtml(typeof d.userAnswer === 'string' ? d.userAnswer : '未作答')}</div>
              </div>
              <div style="margin-top:12px;padding:14px;background:var(--primary-50);border-radius:8px;color:var(--primary-dark)">
                <div style="font-weight:600;margin-bottom:8px">参考答案：</div>
                <div style="white-space:pre-wrap;font-size:14px;line-height:1.7">${escapeHtml(d.referenceAnswer || d.correctAnswer || '未设置')}</div>
              </div>
              ${d.analysis ? `<div style="margin-top:8px;padding:10px;background:var(--surface-2);border-radius:6px;font-size:13px"><strong>💡 解析：</strong>${escapeHtml(d.analysis)}</div>` : ''}
              ${subjectivePending || d.earned === 0 ? `
              <div style="margin-top:12px;display:flex;align-items:center;gap:8px">
                <label style="font-weight:600">评分：</label>
                <input type="number" class="form-input" id="subj_score_${recordId}_${i}" value="${d.earned}" min="0" max="${d.score}" style="width:80px" />
                <span style="font-size:13px;color:var(--text-muted)">/ ${d.score} 分</span>
              </div>` : ''}
            </div>`;
          }
          // 客观题渲染
        ...
        ${subjectivePending ? `<div style="margin-top:16px;padding:16px;background:var(--success-light);border-radius:8px;text-align:center">
          <button class="btn btn-success" onclick="Reports.gradeSubjectiveQuestions('${recordId}')">✅ 确认评分并更新成绩</button>
        </div>` : ''}
      `,
      okText: subjectivePending ? '评分' : '⬇️ 导出成绩单',
      cancelText: '关闭',
      onOk: () => {
        if (subjectivePending) {
          gradeSubjectiveQuestions(recordId);
          return false;
        }
  ...
  // ---------- 主观题评分 ----------
  function gradeSubjectiveQuestions(recordId) {
    const r = DB.getRecordById(recordId);
    if (!r) return;
    const details = r.details || [];
    ...
    let allGraded = true;
    details.forEach((d, i) => {
      if (d.subjective || subjectiveTypes.includes(d.type)) {
        const scoreInput = document.getElementById(`subj_score_${recordId}_${i}`);
        if (scoreInput) {
          const earned = parseInt(scoreInput.value) || 0;
          d.earned = Math.min(earned, d.score);
          subjectiveScore += d.earned;
        } else {
          allGraded = false;
        }
      }
    });
    if (!allGraded) { toast('请为所有主观题评分', 'error'); return; }
    // 更新记录
    const totalScore = r.score + subjectiveScore; // 客观题分 + 主观题分
    const scorePct = Math.round(totalScore / r.totalScore * 100);
    const passed = scorePct >= r.passScore;
    DB.updateRecord(recordId, {
      score: totalScore,
      passed,
      subjectiveGraded: true,
      subjectiveScore,
      details,
    });
    DB.addLog('grade_subjective', recordId, `评分记录「${r.userName} - ${r.examTitle}」，主观题得分 ${subjectiveScore}`);
    toast(`评分完成！总得分 ${totalScore}/${r.totalScore}（${scorePct}%），${passed ? '通过' : '未通过'}`, 'success');
    App.render();
  }
  ...
  function exportRecords(records, fileName) {
    if (typeof XLSX === 'undefined') { toast('XLSX 库未加载，请刷新页面重试', 'error'); return; }
    ...
    const data = records.map(r => {
      ...
      return {
        ...
        '得分率(%)': pct,
        '及格线(%)': r.passScore,
        '是否及格': r.subjectiveGraded ? (r.passed ? '是' : '否') : '待评分',
        '答对题数': r.correctCount || '',
        '总题数': r.questionCount || '',
        '含主观题': r.hasSubjective ? '是' : '否',
        '主观题得分': r.subjectiveScore || 0,
  ...
  function exportSingleRecord(recordId) {
    ...
    const info = [{
      ...
      { '项目': '及格线', '内容': `${r.passScore}%` }, { '项目': '考试结果', '内容': r.subjectiveGraded ? (r.passed ? '通过' : '未通过') : '待评分' },
      { '项目': '答对题数', '内容': `${r.correctCount || 0} / ${r.questionCount || 0}` },
      { '项目': '含主观题', '内容': r.hasSubjective ? '是' : '否' },
      { '项目': '主观题得分', '内容': r.subjectiveScore || 0 },
    ...
    if (r.details && r.details.length) {
      const detailData = r.details.map((d, i) => {
        ...
        return {
          ...
          '题目': d.stem,
          '你的答案': isSubj ? (typeof d.userAnswer === 'string' ? d.userAnswer : '未作答') : (d.userAnswer.length ? d.userAnswer.map(a => String.fromCharCode(65 + a)).join(',') : '未作答'),
          '参考答案': isSubj ? (d.referenceAnswer || '') : d.correctAnswer.map(a => String.fromCharCode(65 + a)).join(','),
          '是否正确': isSubj ? (d.earned > 0 ? `得分${d.earned}/${d.score}` : '待评分') : (d.isCorrect ? '正确' : '错误'),
          '得分': `${d.earned} / ${d.score}`,
          '解析': d.analysis || '',
        };
      });
  ...
  return {
    records, setFilter, delRecord, viewRecord, gradeSubjectiveQuestions,
    showAnalytics,
